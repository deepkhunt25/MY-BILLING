    ...
    <div class="page-enter">
      ...
      : `<div class="logo-placeholder">
            ...
            <div class="form-group">
              ...
              <div class="logo-upload-area" id="signatureUploadArea" onclick="document.getElementById('signatureFileInput').click()" style="height: 90px; cursor: pointer;">
                ${business.signature
      ? `<img src="${business.signature}" alt="Signature" style="max-height: 70px; max-width: 200px; object-fit: contain;">`
      : `<div class="logo-placeholder">
                        <span style="font-size: 1.5rem;">✍️</span>
                        <span style="font-size: 0.82rem; color: var(--text-muted);">Click to upload signature</span>
                      </div>`
    }
              </div>
              <input type="file" id="signatureFileInput" accept="image/*" style="display: none;" onchange="handleSignatureUpload(event)">
              ${business.signature ? `
                <button type="button" class="btn btn-sm btn-outline" style="margin-top: 8px;" onclick="handleRemoveSignature()">
                  ✕ Remove Signature
                </button>
              ` : ''}
              <p style="font-size: 0.75rem; color: var(--text-muted); margin-top: 4px;">Accepted: JPG, PNG, or any image. Use a transparent-background PNG for best results.</p>
            </div>
...
function handleSaveBusiness(e) {
  ...
  saveBusiness({
    ...
    signature: current.signature // preserve signature
  });
...
function handleSignatureUpload(e) {
  ...
  if (!file) return;
  if (file.size > 500 * 1024) {
    showToast('Signature image is large (>500KB). Consider a smaller image.', 'info');
  }
  const reader = new FileReader();
  reader.onload = function (event) {
    const business = getBusiness();
    business.signature = event.target.result;
    saveBusiness(business);
...
function handleSaveUpi(editId) {
  ...
  if (editId) {
    updateUpiAccount(editId, { label, upiId, qrDataUrl });
    showToast('UPI account updated!');
  } else {
    addUpiAccount({ label, upiId, qrDataUrl });

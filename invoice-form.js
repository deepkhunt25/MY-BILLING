...
function handleSaveInvoice(e) {
  ...
  const selectedStatus = document.getElementById('invStatus').value; // user's explicit choice
  // If a received amount is entered, auto-derive from balance; otherwise honour the dropdown
  const status = receivedAmountForStatus > 0
    ? (balanceForStatus > 0 ? 'due' : 'paid')
    : selectedStatus;
